...
export class AwsWebsiteCdnCdkStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // Viewer-request CloudFront Function (cf/normalize.js): writes the x-cache-variant
    // cache-key token, prepended to the key allowlist below (Tier 1, live since 2026-07-01).
    ...
    // Standard logging v2 (CloudWatch vended-log delivery), parallel with the legacy
    // stream above until the blog-side ingest cutover (PII-free stats plan:
    // projects/blog/queue § Analytics). Prefix rides in the destination ARN;
    // outputFormat is frozen at creation — delete + recreate to change it.
