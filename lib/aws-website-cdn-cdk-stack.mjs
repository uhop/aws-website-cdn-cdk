    ...
    const distribution = new cloudfront.CfnDistribution(this, 'aws-website-cdn-cdk-distribution', {
      distributionConfig: {
        ...
        defaultCacheBehavior: {
          ...
          // Cache key = Accept + Accept-Encoding only (the variant-negotiation axes).
          // NOT Origin/Referer: legacy ForwardedValues keys on whatever it forwards, so
          // those two shredded the cache (~60% HTML miss) for zero use. Don't re-add for CORS.
          forwardedValues: {
            queryString: false,
