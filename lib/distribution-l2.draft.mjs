...
// Replicates the live legacy ForwardedValues cache key. Raw Accept-Encoding is
// intentional — see header note (zstd/avif selection needs the unnormalized value).
const cachePolicy = new cloudfront.CachePolicy(this, 'cf-cache-policy', {
  cachePolicyName: 'lazutkin-blog-negotiation',
  comment: 'Accept/Accept-Encoding/Origin/Referer in cache key (variant negotiation)',
  minTtl: cdk.Duration.seconds(0),
  defaultTtl: cdk.Duration.seconds(86400), // 1 day
  maxTtl: cdk.Duration.seconds(31536000), // 1 year
  headerBehavior: cloudfront.CacheHeaderBehavior.allowList('Accept', 'Accept-Encoding', 'Origin', 'Referer'),
...
// OPTIONAL cost/perf win (separate, validated change): `Referer` in the cache
// key fragments the cache hard — every distinct referer is a separate cached
// object — and the origin doesn't need it (CloudFront logs cs(Referer)
// regardless). Dropping it from the allowList above raises the hit ratio and
// trims origin hits. Confirm nothing downstream reads Referer before removing.
